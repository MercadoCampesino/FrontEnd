...
export const CartProvider = ({ children }) => {
  ...
  const increaseFromCart = (idProducto) => {
    const updatedCart = cart.map(item => {
      if (item.idProducto === idProducto) {
        if (item.counter > 1) {
          return {
  ...
  const removeProductFromCart = (idProducto) => {
    const updatedCart = cart.filter(item => item.idProducto !== idProducto);
    setCart(updatedCart);
  };

  return (
    <CartContext.Provider value={{ cart, addToCart, removeFromCart, removeProductFromCart, increaseFromCart }}>
      {children}
